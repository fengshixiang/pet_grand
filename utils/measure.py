...
def compute_measure(y, pred, data_range, length):
    psnrs, ssims, nmses = 0., 0., 0.
    for i in range(y.shape[0]):
        psnr, ssim, nmse = compute_measure_single(y[i:i+1], pred[i:i+1], data_range)
        psnrs+=psnr/length
        ssims+=ssim/length
        nmses+=nmse/length
    return psnrs, ssims, nmses    


def compute_measure_single(y, pred, data_range):
    psnr = compute_PSNR(pred, y, data_range)
    ssim = compute_SSIM(pred, y, data_range)
    nmse = compute_NMSE(pred, y)
...
def compute_NMSE(y, pred):
    if type(y) == torch.Tensor:
        mse = compute_MSE(y, pred)
        variance = compute_MSE(y, torch.mean(y))
        return (mse / variance).item()
    else:
        mse = compute_MSE(y, pred)
        variance = compute_MSE(y, np.mean(y))
        return mse / variance
